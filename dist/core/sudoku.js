...
export function createSourceBoard(sourceString) {
    const boxes = Array.from({ length: 9 }, () => []);
    for (let i = 0; i < 81; i++) {
        const char = sourceString[i];
        const val = (char === "." || char === "0") ? null : char;
...
export function showBoard() {
    ...
    if (!board)
        return false;
    const boardEntry = board.querySelector(".board-entry.read-only");
...
export function highlightImpact(entry) {
    const position = getPosition(entry);
    if (!position)
        return;
    const board = document.getElementById("current-board");
    board?.querySelectorAll(`.box-${position.box}, .row-${position.row}, .col-${position.col}`).forEach(div => {
...
export function removeImpact() {
    const board = document.getElementById("current-board");
    board?.querySelectorAll(".impact").forEach(div => {
        div.classList.remove("impact");
    });
}
/** Changes focus to given position of entry. */
export function changeFocusTo(row, col, addImpact = true) {
    removeImpact();
    const board = document.getElementById("current-board");
    board?.querySelectorAll(".focused").forEach(element => {
        element.classList.remove("focused");
    });
    const focusedBox = board.querySelector(`.board-entry.row-${row}.col-${col}`);
    if (!focusedBox)
        return;
    focusedBox.classList.add("focused");
    checkEntry(focusedBox, false);
    if (addImpact)
        highlightImpact(focusedBox);
}
export function moveFocus(direction, mag = 1) {
    const board = document.getElementById("current-board");
    const focused = board?.querySelector(".focused");
    if (!focused) {
        changeFocusTo(1, 1);
        return;
    }
    ...
    if (direction === "u")
        row = Math.max(1, row - mag);
    if (direction === "d")
        row = Math.min(9, row + mag);
    if (direction === "l")
        col = Math.max(1, col - mag);
    if (direction === "r")
        col = Math.min(9, col + mag);
    changeFocusTo(row, col);
}
//#endregion UI
//#region Logic
export function getPosition(entry) {
    if (!entry.classList.contains("board-entry"))
        return;
    return {
...
export function addEntry(num, validate = true) {
    if (!gameState.boardState)
        return;
    const board = document.getElementById("current-board");
    board.querySelectorAll(".focused").forEach(entry => {
        if (entry.classList.contains("read-only"))
            return;
        ...
        if (!position)
            return;
        gameState.boardState.currentState[Number(position.box) - 1][3 * Number(position.row) + Number(position.col) - 4] = num;
        if (validate)
            checkEntry(entry);
    });
}
/** Checks if the entry is valid. `force` = true, forces it to check the impact zone even if the entry is empty. */
export function checkEntry(entry, force = true) {
    if (!gameState)
        return;
    if (entry.classList.contains("read-only"))
        return;
    ...
    if (!span)
        return;
    if (!force && !span.textContent)
        return;
    const board = document.getElementById("current-board");
    const impactZone = board.querySelectorAll(`.box-${position.box}, .row-${position.row}, .col-${position.col}`);
    impactZone.forEach(impactEntry => {
        if (impactEntry == entry)
            return;
